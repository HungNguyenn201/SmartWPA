...
| `filters.direction.sectors_number` | int | Không | 12 | Số sector (1–72) |
| `filters.direction.sector_ids` | int[] | Không | — | Chỉ giữ sector nào (0-based hoặc 1-based): `[0, 1, 2]` |
...
// → 400, code: INVALID_PARAMETERS (group_by turbine không hỗ trợ ở turbine endpoint; dùng farm endpoint nếu cần)
```

---

### 4.13 Cross Data Analysis (Farm)
...
Cross turbine analysis cấp farm (manual 1.3.5.3 b): so sánh X/Y nhiều turbine, hỗ trợ **Data splitting** qua `group_by`. **Luôn** trả dạng curve (binned theo trục X, mặc định 50 bin) khi X ≠ wind_direction; khi **X = wind_direction** thì response có **wind_rose** (sector aggregation). Không có tham số `output` hay `curve_bins` trong request.

**Data splitting (Manual 1.3.5.3 b: Split chart series by)** — **Farm** không có `classification` (chỉ turbine có). Farm hỗ trợ **`group_by`**:
- `none`: không chia nhóm.
- `turbine`: mỗi turbine một series; wind_rose có `by_turbine`.
- `monthly` / `seasonally` / `yearly`: nhóm theo tháng/quý/năm (Jan, Q1, 2023…); curve có `points_by_group`, wind_rose có `by_group`.
- `hourly` / `day_night`: nhóm theo giờ (H00..H23) hoặc Day/Night; curve có `points_by_group`, wind_rose có `by_group`.
- `source`: nhóm theo giá trị source thứ 3 (binning, cấu hình trong `group`).

**Tham số:** Giống turbine cross-data (x_source, y_source, group_by, datetime, filters, …), thêm:

| Field | Type | Bắt buộc | Default | Mô tả |
|-------|------|----------|---------|-------|
| `turbine_ids` | int[] | Không | toàn bộ turbine | Chỉ lấy turbine trong farm theo id |
| `max_points_per_turbine` | int | Không | 10000 | Giới hạn điểm mỗi turbine (100–50000) |

**`group_by` (farm):** `none` \| `turbine` \| `monthly` \| `seasonally` \| `yearly` \| `hourly` \| `day_night` \| `source`. **Không** có `classification`.

**`filters.day_night`:** Thống nhất với turbine: chỉ `"day"` \| `"night"` \| `""`.

**Filters (Manual: Date time + Advanced filters)**\n\n- **Filter** là các điều kiện **loại bớt dữ liệu** trước khi tính curve/wind_rose.\n- **`group_by`** là **data splitting** (chia series/wind_rose theo nhóm). Farm **không** dùng `filters.classifications` (classification chỉ turbine).\n\nCác filter: **Date time** (`datetime.start_time_ms`, `end_time_ms`, `start_hour`, `end_hour`), **Month** (`filters.months`), **Day/Night** (`filters.day_night` = \"day\" | \"night\" | \"\"), **Direction** (`filters.direction`), **Source min/max** (`filters.ranges`).\n\n**Ví dụ 1 — Lọc temperature:**\n\n```json\n{\n  \"x_source\": \"temperature\",\n  \"y_source\": \"power\",\n  \"group_by\": \"none\",\n  \"filters\": {\n    \"ranges\": [\n      { \"source\": \"temperature\", \"min\": -40, \"max\": 60 }\n    ]\n  }\n}\n```\n\n**Ví dụ 2 — Lọc sector hướng gió:**\n\n```json\n{\n  \"x_source\": \"wind_speed\",\n  \"y_source\": \"power\",\n  \"group_by\": \"turbine\",\n  \"filters\": {\n    \"direction\": {\n      \"source\": \"wind_direction\",\n      \"sectors_number\": 12,\n      \"sector_ids\": [1, 2, 3]\n    }\n  }\n}\n```\n\n**Ví dụ 3 — Wind rose theo tháng (group_by=monthly) + chỉ ban đêm:**\n\n```json\n{\n  \"x_source\": \"wind_direction\",\n  \"y_source\": \"power\",\n  \"group_by\": \"monthly\",\n  \"datetime\": { \"start_time_ms\": 1325376000000, \"end_time_ms\": 1356998400000 },\n  \"filters\": { \"day_night\": \"night\" }\n}\n```\n+\n **Response (200):** Farm cross-data **không trả regression**; **luôn** trả dạng **binned points** hoặc **wind_rose**. Không có tham số `output`/`curve_bins` trong request.
**Response (200):** Farm cross-data **không trả regression**; **luôn** trả dạng **binned points** hoặc **wind_rose**. Không có tham số `output`/`curve_bins` trong request.
...
- Khi **X = wind_direction**:
  - `data.wind_rose` = `{ sectors_number, direction_source, sectors: [...], by_turbine?: [...], by_group?: { "Jan": [...], ... } }`. `by_turbine` khi `group_by=turbine`; `by_group` khi `group_by` ∈ {monthly, seasonally, yearly, hourly, day_night}.
  - `data.points` rỗng.
...
#### TH Farm 2: Wind rose (x_source=wind_direction)
...
```json
{
  ...
  "y_source": "power",
  "datetime": { "start_time_ms": 1325376000000, "end_time_ms": 1356998400000 },
  "filters": { "direction": { "source": "wind_direction", "sectors_number": 12 } }
}
```

**Response (200) — TH Farm 2 (không group_by=turbine):**
...
```json
{
  ...
  "data": {
    ...
    "summary": {
      "rows_before_filters": 105120,
      "rows_after_filters": 102000,
      "points_returned": 0,
      "turbines_requested": [1, 2],
      "turbine_count": 2
    },
    "points": [],
    "wind_rose": {
      "sectors_number": 12,
      "direction_source": "wind_direction",
      "sectors": [
        {
          "sector_id": 0,
          "start_deg": 0.0,
...
**Response (200) — TH Farm 2 với group_by=turbine (có by_turbine):**

Khi request thêm `"group_by": "turbine"`, `data.wind_rose` có thêm mảng `by_turbine`:
...
```json
{
  ...
  "data": {
    ...
    "y_source": "power",
    "group_by": "turbine",
    "period": { "start_time_ms": 1325376000000, "end_time_ms": 1356998400000 },
    "summary": {
      "rows_before_filters": 105120,
      "rows_after_filters": 102000,
      "points_returned": 0,
    ...
    "wind_rose": {
      ...
      "direction_source": "wind_direction",
      "sectors": [
        { "sector_id": 0, "start_deg": 0.0, "end_deg": 30.0, "center_deg": 15.0, "count": 4200, "y_mean": 1240.0, "y_min": 0.0, "y_max": 2480.0, "y_median": 1170.0 },
        { "sector_id": 1, "start_deg": 30.0, "end_deg": 60.0, "center_deg": 45.0, "count": 4100, "y_mean": 1310.0, "y_min": 0.0, "y_max": 2460.0, "y_median": 1235.0 }
      ],
      "by_turbine": [
        {
          "turbine_id": 1,
          "turbine_name": "WT1",
          "sectors": [
            { "sector_id": 0, "start_deg": 0.0, "end_deg": 30.0, "center_deg": 15.0, "count": 4260, "y_mean": 1255.2, "y_min": 0.0, "y_max": 2500.0, "y_median": 1185.0 },
            { "sector_id": 1, "start_deg": 30.0, "end_deg": 60.0, "center_deg": 45.0, "count": 4050, "y_mean": 1325.0, "y_min": 0.0, "y_max": 2480.0, "y_median": 1242.0 }
          ]
        },
        {
          "turbine_id": 2,
          "turbine_name": "WT2",
          "sectors": [
            { "sector_id": 0, "start_deg": 0.0, "end_deg": 30.0, "center_deg": 15.0, "count": 4260, "y_mean": 1245.8, "y_min": 0.0, "y_max": 2480.0, "y_median": 1175.5 },
            { "sector_id": 1, "start_deg": 30.0, "end_deg": 60.0, "center_deg": 45.0, "count": 4050, "y_mean": 1315.6, "y_min": 0.0, "y_max": 2460.0, "y_median": 1228.0 }
          ]
        }
      ]
    }
...
> **Kiểm tra:** `data.wind_rose.sectors` có 12 phần tử (sector_id 0..11), mỗi sector có `sector_id`, `start_deg`, `end_deg`, `center_deg`, `count`, `y_mean`, `y_min`, `y_max`, `y_median`. Khi `group_by=turbine`, `wind_rose.by_turbine` là mảng theo turbine với `turbine_id`, `turbine_name`, `sectors`. `data.points` rỗng khi output wind_rose.

---

#### TH Farm 3: Lọc turbine_ids
...
> **Kiểm tra:** Chỉ dữ liệu turbine 1 và 2; `summary.turbines_requested` = [1, 2], `turbine_count` = 2. Mỗi point có `turbine_id` (1 hoặc 2).

---

#### TH Farm 4: Lỗi — farm không có turbine / turbine_ids không thuộc farm

**Request:** `turbine_ids: [99, 100]` (id không thuộc farm).
...
{
  "success": false,
  "error": "No matching turbines for the given turbine_ids",
  "code": "INVALID_PARAMETERS"
}
```

#### TH Farm 5: Lỗi — thiếu dữ liệu

**Request:** time range không có SCADA hoặc computation.
...
{
  "success": false,
  "error": "No data available for the specified time range and turbines",
  "code": "NO_DATA"
}
```
...
### 4.14 Monthly Dashboard (Turbine)
...
Cross Data Analysis: turbine level (4.12) và farm level (4.13). Farm hỗ trợ `group_by=turbine` và wind rose khi `x_source=wind_direction`.
