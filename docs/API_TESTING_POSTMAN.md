...
**Data splitting (Manual: Split chart series by)** tương ứng tham số **`group_by`**:
- `none`: không chia nhóm.
- `turbine`: mỗi turbine một series (MP1, MP2, …); mỗi point/series có `turbine_id`.
- `classification`: nhóm theo trạng thái (NORMAL, STOP, …).
- `monthly` / `yearly` / `seasonally`: nhóm theo tháng/năm/quý (profile).
