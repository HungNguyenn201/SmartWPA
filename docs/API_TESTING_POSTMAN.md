...
**Tham số:** Giống turbine cross-data (x_source, y_source, group_by, datetime, filters, regression, …), thêm:
...
**Response (200):** Cấu trúc đồng nhất với turbine: `success`, `data` với `farm_id`, `farm_name`, `x_source`, `y_source`, `group_by`, `regression`, `period` (start_time_ms, end_time_ms), `summary` (rows_before_filters, rows_after_filters, points_returned, **turbines_requested**, **turbine_count**), `points` (mỗi point có **turbine_id** khi nhiều turbine). Khi X = wind_direction (output auto/curve): `data.wind_rose` = `{ sectors_number, direction_source, sectors: [{ sector_id, start_deg, end_deg, center_deg, count, y_mean, y_min, y_max, y_median }], by_turbine?: [...] }`; `points` có thể rỗng.
