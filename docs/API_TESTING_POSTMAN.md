...
Cross turbine analysis cấp farm (manual 1.3.5.3 b): so sánh X/Y nhiều turbine, hỗ trợ `group_by=turbine`, và khi **X = wind_direction** thì response có **wind_rose** (sector aggregation).
...
| `output` | string | Không | `"auto"` | `"auto"` \| `"scatter"` \| `"curve"`. Auto: nếu x_source=wind_direction → wind_rose |

**`group_by` (farm):** Thêm giá trị `turbine` (nhóm theo turbine). Các giá trị khác giống turbine.

**`filters.day_night`:** Thống nhất với turbine: chỉ `"day"` \| `"night"` \| `""`.

**Response (200):** Farm cross-data **không trả regression** (đúng manual, payload nhẹ). Response gồm: `success`, `data` với `farm_id`, `farm_name`, `x_source`, `y_source`, `group_by`, `period` (start_time_ms, end_time_ms), `summary` (rows_before_filters, rows_after_filters, points_returned, **turbines_requested**, **turbine_count**), `points` (mỗi point có **turbine_id** khi nhiều turbine). Khi X = wind_direction (output auto/curve): `data.wind_rose` = `{ sectors_number, direction_source, sectors: [{ sector_id, start_deg, end_deg, center_deg, count, y_mean, y_min, y_max, y_median }], by_turbine?: [...] }`; `points` có thể rỗng.

#### TH Farm 1: Scatter nhiều turbine (group_by=turbine)
...
> **Kiểm tra:** Mỗi phần tử trong `data.points` có `timestamp_ms`, `x`, `y`, `group` (tên turbine), `turbine_id`. `data.summary` có `turbines_requested`, `turbine_count`. Không có `wind_rose`.
