...
- Farm cross-data: **không có regression**; response gồm metadata (farm), `x_source`, `y_source`, `group_by`, `period`, `summary`. Khi X ≠ wind_direction: luôn `curve_series` (mảng series với bins, 50 bin mặc định), `points` = [], `summary.curve_series_count`; khi X = wind_direction: `wind_rose`. Thêm `summary.turbines_requested`, `summary.turbine_count`; tùy chọn `statistics`.
