...
Cross Data Analysis có **turbine** (`POST /api/turbines/{id}/cross-data-analysis/`) và **farm** (`POST /api/farms/{id}/cross-data-analysis/`). Farm hỗ trợ **Data splitting** qua `group_by` (manual: Split chart series by), gồm `turbine`, `classification`, `monthly`, `yearly`, `seasonally`, `source`. Khi `x_source=wind_direction` trả thêm `wind_rose` (sector aggregation). Khi X ≠ wind_direction farm **luôn** trả dạng **series theo turbine** (giống `power_curve`): `series: [{ turbine_id, turbine_name, points | points_by_group }]` với points là dữ liệu **binned** (mặc định 50 bin) để vẽ curve. Không có tham số `output`/`curve_bins` trong request. **Farm không trả regression**.
