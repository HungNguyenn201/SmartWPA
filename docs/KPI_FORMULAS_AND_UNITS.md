...
**Response:** `data.regression` = object key theo type: `{ "linear": { type, coefficients, equation, r2, rmse, enabled, line_points }, "polynomial2": { ... }, ... }`.

- `line_points`: danh sách điểm `{x, y}` (polyline, mặc định ~200 điểm/đường) để FE vẽ trực tiếp, không cần parse `equation`.
- Khi có `group_by`: `data.regressions_by_group` = `{ "nhãn_nhóm": { "linear": { ... }, "polynomial2": { ... }, ... }, ... }` (mỗi group cũng có `line_points` theo từng type).

### 13.2 Group by options

**Turbine** cross-data: `none` \| `classification` \| `monthly` \| `seasonally` \| `yearly` \| `source`. **Farm** cross-data (manual 1.3.5.3 b): `none` \| `turbine` \| `monthly` \| `seasonally` \| `yearly` \| `hourly` \| `day_night` \| `source` — **không** có `classification`.

| group_by | Turbine | Farm | Mô tả |
|----------|---------|------|-------|
| `none` | ✓ | ✓ | Không gom nhóm |
| `classification` | ✓ | — | Tô màu theo trạng thái (NORMAL, STOP, …); **chỉ turbine** |
| `turbine` | — | ✓ | Mỗi turbine một series; wind_rose có `by_turbine` |
| `monthly` | ✓ | ✓ | Profile tháng (Jan, Feb, …); wind_rose có `by_group` (farm) |
| `seasonally` | ✓ | ✓ | Profile quý (Q1–Q4); wind_rose có `by_group` (farm) |
| `yearly` | ✓ | ✓ | Theo năm (2023, 2024); wind_rose có `by_group` (farm) |
| `hourly` | — | ✓ | Theo giờ (H00..H23); **chỉ farm**; wind_rose có `by_group` |
| `day_night` | — | ✓ | Day / Night; **chỉ farm**; wind_rose có `by_group` |
| `source` | ✓ | ✓ | Trục Z: bin theo giá trị liên tục của 1 source |

Cross Data Analysis có **turbine** (`POST /api/turbines/{id}/cross-data-analysis/`) và **farm** (`POST /api/farms/{id}/cross-data-analysis/`). Farm **Data splitting** (manual 1.3.5.3 b): `monthly`, `seasonally`, `yearly`, `hourly`, `day_night` (+ `turbine`, `source`); **không** có classification. Khi `x_source=wind_direction` farm trả `wind_rose`; có thể có `by_turbine` (khi `group_by=turbine`) hoặc `by_group` (khi `group_by` ∈ monthly/seasonally/yearly/hourly/day_night). Khi X ≠ wind_direction farm trả **series theo turbine** (binned, mặc định 50 bin). **Farm không trả regression**.

**Response:** API không trả metadata đơn vị (`units`) hay nguồn dữ liệu (`data_source_used`) trong output.

- Turbine cross-data: có `regression` (object key theo type) và (khi có group_by) `regressions_by_group`.
- Farm cross-data: **không có regression**; response gồm metadata (farm), `x_source`, `y_source`, `group_by`, `period`, `summary`. Khi X ≠ wind_direction: luôn `series` (per turbine) và `summary.points_returned` là tổng số point binned; khi X = wind_direction: `wind_rose` (và `points` rỗng). Thêm `summary.turbines_requested`, `summary.turbine_count`; tùy chọn `statistics`.

### 13.3 Classification filter (turbine only)

- **Turbine** cross-data: `filters.classifications` danh sách trạng thái cần giữ (vd `["NORMAL", "CURTAILMENT"]`); `only_computation_data`: khi `true` chỉ dùng ClassificationPoint.
- **Farm** cross-data **không** hỗ trợ classification (không có `group_by=classification`, không lọc theo `filters.classifications`).
...
- **Group "Time profile Yearly"**: manual có Time profile → Monthly, Seasonally, Yearly; API dùng `monthly`, `seasonally`, `yearly` (không còn alias time_profile_*).
