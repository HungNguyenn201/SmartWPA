...
- Farm cross-data: **không có regression**; response gồm metadata (farm), `x_source`, `y_source`, `group_by`, `period`, `summary`, `points`; thêm `summary.turbines_requested`, `summary.turbine_count`, mỗi point có `turbine_id` khi nhiều turbine; khi wind rose có `wind_rose`. Tùy chọn `statistics`.
